    ...
    **************************************/
	...
	_insertToList: function(node, list) {
		...
		var _addNode = function() {
			...
			try {
				/*------- Old Functionality -------*/
				// This 'if' sorts selected options in alphabetical order by default
				// if (succ) {
				...
				// Show selected options in selected options list
				list.append(node);
...
$.extend($.ui.multiselect, {
	...
	locale: {
		...
		itemsCount:'#{count} seleccionados',
		itemsTotal:'#{count} en Total',
		...
		errorRequest:"Perdon! Hay un problema con la llamada remota. (Type: #{status})"
	},
	constants: {
		MESSAGE_WARNING: 0,
		MESSAGE_EXCEPTION: 1,
